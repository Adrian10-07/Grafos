...
const agregarUbicaciones = () => {
    ...
    if (newUbicacion == "") {
        ...
        alert("Ciudad añadida exitosamente");
        imprimirUbicaciones();
    }
};

const anadirAristas = () => {
    let distancia = Number(document.getElementById("getWeight").value);
...
const encontrarCaminoMasCorto = () => {
    ...
    console.log(`El camino más corto de ${inicio} a ${fin} es: ${path.join(" -> ")} con una distancia de ${distance}km`);
    ...
    resultado.textContent = `El camino más corto de ${inicio} a ${fin} es: ${path.join(" -> ")} con una distancia de ${distance}km`;
