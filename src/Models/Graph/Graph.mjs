import ListaEnlazada from "../ListaEnlazada/ListaEnlazada.mjs";
...
export default class Graph {
    ...
    addConexion(start, end, weight = 1) {
        if (this.#map.has(start) && this.#map.has(end)) {
            ...
            this.#listaAdyacencia[this.#map.get(start)].push(this.#map.get(end), numericWeight);
            this.#listaAdyacencia[this.#map.get(end)].push(this.#map.get(start), numericWeight); // Conexión en la dirección opuesta

            if (!this.#matrizAdyacencia[this.#map.get(start)]) {
                this.#matrizAdyacencia[this.#map.get(start)] = [];
            }
            if (!this.#matrizAdyacencia[this.#map.get(end)]) {
                this.#matrizAdyacencia[this.#map.get(end)] = [];
            }

            this.#matrizAdyacencia[this.#map.get(start)][this.#map.get(end)] = numericWeight;
            this.#matrizAdyacencia[this.#map.get(end)][this.#map.get(start)] = numericWeight; // Conexión en la dirección opuesta
            return true;
        }
        return false;
    }
    ...
    dijkstra(start, end, callback) {
        ...
        for (let i = 0; i < this.#listaAdyacencia.length; i++) {
            ...
            if (minVertex === -1) break; // Todos los vértices accesibles han sido visitados
            visited[minVertex] = true;

            if (callback) callback(this.#getKeyFromIndex(minVertex));

            const adjacencyList = this.#listaAdyacencia[minVertex];
            if (!adjacencyList) continue; // Si no hay lista de adyacencia, saltar
